...
internal class QaAsServiceSteps : SpecFlowContext
{
    ...
    {
        _googleAnalyticsRequestData.Value = RunActionAndGetGoogleAnalyticsRequest(() =>
        {
    ...
    {
        _page.Init<QaAsServicePage>().GetElementFromMenuCategoryByText(categoryName, pageName).ClickAsync().GetAwaiter().GetResult();
