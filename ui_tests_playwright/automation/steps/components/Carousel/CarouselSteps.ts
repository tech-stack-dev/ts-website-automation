...
export class CarouselSteps {
    ...
    public async checkInfoCarouselItem(order: string, title: string, carouselIdentifier: string) {
        ...
        let infoCarouselItems = await section.getByTestId(CarouselIds.InfoCarousel_Item).all();
        let index = Number(order) - 1;
        expect(await infoCarouselItems[index].locator(CarouselIds.InfoCarousel_Order).textContent()).toContain(order);
        expect(await infoCarouselItems[index].locator(CarouselIds.InfoCarousel_Title).textContent()).toContain(title);
    }

    public async checkPhotosAmountInPhotoCarousel(amount: number, sectionIdentifier: string) {
        let section = await driver.component(ContainerById, sectionIdentifier);
        let photos = await section.getByTestId(Carousel.PhotoCarousel_Item).all();
