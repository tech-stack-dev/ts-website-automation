...
class ContainerSteps {
    ...
    public async clickContainer(type:any, identifier: string,  times: number = 1, parent?: Locator){
        await (await driver.component(type, identifier, parent)).click({clickCount: times, delay: 50});
    }
    ...
    public async checkContainerContainsClassProperty(type: any, identifier: string, prop: string, parent?: Locator) {
    ...
    public async checkContainerSectionTitle(identifier: string, expectedTitle: string, parent?: Locator) {
        let title = await (await driver.component(ContainerById, identifier, parent)).section_title;
        await expect(title).toHaveText(expectedTitle);
    }
    ...
    public async checkContainerText(identifier: string, expectedTitle: string, parent?: Locator, skipLineBrakes=true) {
