...
test(`Check the footer information from the 'Footer' container on all pages @Regression @Footer @TSWEB-655 @TSWEB-674`, async () => {
...
test(`Check the redirection by the "Techstack" logo on all pages @Regression @Footer @TSWEB-655`, async () => {
	for (const url of testDataProvider) {
		await baseDriverSteps.goToUrl(url);
...
test(`Check the redirection for the Industries block on all pages @Regression @Footer @TSWEB-833`, async () => {
	for (const url of testDataProvider) {
		await baseDriverSteps.goToUrl(url);
...
test(`Check the redirection for the Services block on all pages @Regression @Footer @TSWEB-655`, async () => {
	for (const url of testDataProvider) {
		await baseDriverSteps.goToUrl(url);
...
test(`Check the redirection for the Company block on all pages @Regression @Footer @TSWEB-655 @TSWEB-674`, async () => {
	const companyList = new Map([
...
test(`Check the redirection by the "Contact us" button on all pages @Regression @Footer @TSWEB-655`, async () => {
	for (const url of testDataProvider) {
		await baseDriverSteps.goToUrl(url);
...
test(`Check the redirection for the social links on all pages @Regression @Footer @TSWEB-655`, async () => {
...
test(`Check the redirection to the Terms, Cookies Policy, and Sitemap pages on all pages @Regression @Footer @TSWEB-655`, async () => {
	const linkMap = new Map([
