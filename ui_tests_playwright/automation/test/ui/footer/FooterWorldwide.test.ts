...
let footer: Locator;
const testDataProvider = [UrlProvider.webSiteUrl(),
UrlProvider.urlBuilder(UrlPath.ContactUs)
].concat(ContactUsPreconditions.servicesUrlList).concat(ContactUsPreconditions.companyUrlList);
...
for (const url of testDataProvider) {
    ...
    test(`Check the redirection for the social links on the '${url}' link @Regression @Footer @TSWEB-655`, async () => {
        ...
        for (let entries of linkMap.entries()) {
