...
test.skip(
	...
	async () => {
		...
		const actualErrorText_FirstName = driver.getByTestId(Input.FirstName).locator(Input.FieldErrorSelectorCareer);
		const actualErrorText_LastName = driver.getByTestId(Input.LastName).locator(Input.FieldErrorSelectorCareer);
...
test(
	...
	async () => {
		...
		await expect(
			(await driver.component(ContainerByClass, ContainersCareer.SearchResultsTextContainer)).Element
