...
test(`Check cards in 'Custom development services we provide' section on the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await expect(cards.nth(1).getByTestId(CustomSoftwareDevelopent.CardTitle)).toHaveText(
...
test(`Check the content switcher on the 'Technology stack' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await expect(activeContentSection.getByTestId(CustomSoftwareDevelopent.ContentSectionTitle).nth(0)).toHaveText(
		'.NET Stack'
	);
	await expect(
		activeContentSection
			.getByTestId(CustomSoftwareDevelopent.ContentSectionList)
			.nth(0)
			.getByTestId(CustomSoftwareDevelopent.ContentSectionItem)
	).toHaveCount(11);
	await expect(
		activeContentSection
			.getByTestId(CustomSoftwareDevelopent.ContentSectionList)
	...
	await expect(activeContentSection.getByTestId(CustomSoftwareDevelopent.ContentSectionTitle).nth(4)).toHaveText(
...
test(`Check the carousel in the 'Custom software development process' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await driver
		.getByTestId(CustomSoftwareDevelopent.ControlRightButton)
		.click({clickCount: 10, timeout: 3000, force: true});
...
test(`Check the member cards in the 'Custom software development experts' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const cards = driver
		.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentExperts)
		.getByTestId(CustomSoftwareDevelopent.MemberItem);
	await expect(cards).toHaveCount(7);

	for (const member of testData) {
		await expect(cards.nth(member.id).getByTestId(CustomSoftwareDevelopent.MemberPhoto)).toHaveAttribute(
			'src',
			member.memberPhoto
		);
		await expect(cards.nth(member.id).getByTestId(CustomSoftwareDevelopent.MemberDescription)).toHaveText(
			member.memberDescription
		);
		await expect(cards.nth(member.id).getByTestId(CustomSoftwareDevelopent.MemberName)).toHaveText(
			member.memberName
		);
		await expect(cards.nth(member.id).getByTestId(CustomSoftwareDevelopent.MemberLinkLinkedin)).toHaveAttribute(
			'href',
			member.memberLinkLinkedin
		);
		await expect(cards.nth(member.id).getByTestId(CustomSoftwareDevelopent.MemberLinkBlog)).toHaveAttribute(
			'href',
			member.memberLinkBlog
		);
...
test(`Check the 'Case Studies' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const caseCards = driver
		.getByTestId(CustomSoftwareDevelopent.CaseStudies)
		.getByTestId(CustomSoftwareDevelopent.CaseCard);

	for (const caseStudy of testData) {
		const actualCard = caseCards.nth(caseStudy.id);
		await expect(actualCard.getByTestId(CustomSoftwareDevelopent.CaseDomainName)).toHaveText(
			caseStudy.сaseDomainName
		);
...
test(`Check the award cards in 'Our approach to software development' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const actualAwardCardImages = driver
		.getByTestId(CustomSoftwareDevelopent.OurApproachToSoftwareDevelopment)
		.getByTestId(CustomSoftwareDevelopent.AwardCardImage);
...
test(`Check the 'Related artiles' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	const expectedAwards = [
		...
		},
		{
			id: 2,
			articleLeftInfo: 'Software Development',
			articleRightInfo: 'Feb 13',
			articleTitle: 'The Big Data Benefits for Businesses: How to Make Your Company More Competitive',
		},
	];
...
test(`Check the 'FAQ' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	const sections = driver
		.getByTestId(CustomSoftwareDevelopent.Faq)
		.getByTestId(CustomSoftwareDevelopent.SectionContainer);
