...
test("Check the section title and number from the 'Custom software development' block @Regression @CustomSoftwareDevelopent @TSWEB-672", async () => {
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentForYourProduct)
	...
	await expect(
		driver
			...
			.nth(0)
	).toHaveText('Custom software development for your product');
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentServicesWeProvide)
			.getByTestId(Container.ContainerNumber)
			.nth(0)
	).toHaveText('02');
	await expect(
	...
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.TechnologyStack).getByTestId(Container.ContainerNumber).nth(0)
	).toHaveText('03');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.TechnologyStack).getByTestId(Container.ContainerTitle).nth(0)
	).toHaveText('Technology\nstack');
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentBenefits)
			.getByTestId(Container.ContainerNumber)
			.nth(0)
	).toHaveText('04');
	await expect(
		driver
			...
			.getByTestId(Container.ContainerTitle)
			.nth(0)
	).toHaveText('Custom software development benefits');
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentProcess)
			.getByTestId(Container.ContainerNumber)
			.nth(0)
	).toHaveText('05');
	await expect(
		driver
			...
			.getByTestId(Container.ContainerTitle)
			.nth(0)
	).toHaveText('Custom software development process');
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentExperts)
			.getByTestId(Container.ContainerNumber)
			.nth(0)
	).toHaveText('06');
	await expect(
		driver
			...
			.getByTestId(Container.ContainerTitle)
			.nth(0)
	).toHaveText('Custom software\ndevelopment experts');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.CaseStudies).getByTestId(Container.ContainerNumber).nth(0)
	).toHaveText('07');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.CaseStudies).getByTestId(Container.ContainerTitle).nth(0)
	).toHaveText('Case studies');
	await expect(
		driver
			.getByTestId(CustomSoftwareDevelopent.OurApproachToSoftwareDevelopment)
			.getByTestId(Container.ContainerNumber)
			.nth(0)
	).toHaveText('08');
	await expect(
	...
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.Faq).getByTestId(Container.ContainerNumber).nth(0)
	).toHaveText('09');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.Faq).getByTestId(Container.ContainerTitle).nth(0)
	).toHaveText('FAQ');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.RelatedArticles).getByTestId(Container.ContainerNumber).nth(0)
	).toHaveText('10');
	await expect(
		driver.getByTestId(CustomSoftwareDevelopent.RelatedArticles).getByTestId(Container.ContainerTitle).nth(0)
	).toHaveText('Related \narticles');
});
...
test(`Check page title and 'Request a quote' buttons on the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	for (const section of sections) {
		await expect(driver.getByTestId(section).getByTestId(Button.RequestAQuoteButton)).toBeVisible();
	}
});
	...
	for (const section of testData) {
		section.items?.forEach(async function (value, i) {
			const item = driver
				...
				.getByTestId(/ContainerSection/)
				.nth(i);
			await expect(item.getByTestId(Container.SectionTitle)).toHaveText(value);
		});
	}
});
...
test(`Check cards in 'Custom development services we provide' section on the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await expect(cards.nth(1).getByTestId(Container.ContainerNumber)).toHaveText('02');
	await expect(cards.nth(1).getByTestId(Container.ContainerTitle)).toHaveText('Front-End and Back-End development');
	await expect(cards.nth(0).getByTestId(Container.ContainerTitle)).toHaveText('Mobile development');
	await expect(cards.nth(0).getByTestId(Container.ContainerNumber)).toHaveText('01');
	...
	// await cards.nth(0).getByTestId(Container.ContainerTitle).hover();
...
test(`Check the content switcher on the 'Technology stack' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await expect(activeContentSection.getByTestId(Container.ContainerBlock)).toHaveCount(4);
	await expect(activeContentSection.getByTestId(Container.BlockTitle).nth(0)).toBeVisible();
	await expect(activeContentSection.getByTestId(Container.BlockTitle).nth(3)).toBeVisible();

	await expect(activeContentSection.getByTestId(Container.BlockTitle).nth(0)).toHaveText('.NET Stack');
	await expect(
		activeContentSection.getByTestId(Container.ContainerBlock).nth(0).getByTestId(Container.BlockSection)
	).toHaveCount(11);
	await expect(
		activeContentSection.getByTestId(Container.ContainerBlock).nth(0).getByTestId(Container.BlockSection).nth(0)
	).toHaveText('C#');
	...
	await expect(parent.getByTestId(CustomSoftwareDevelopent.ContentSwitcherButton_AiMlDataScience)).toHaveClass(
		/active/
	);
	await expect(parent.getByTestId(CustomSoftwareDevelopent.ContentSwitcherButton_BackEnd)).not.toHaveClass(/active/);

	await expect(activeContentSection.getByTestId(Container.ContainerBlock)).toHaveCount(5);
	await expect(activeContentSection.getByTestId(Container.BlockTitle).nth(4)).toHaveText('Development environment');
	await expect(
		activeContentSection.getByTestId(Container.ContainerBlock).nth(0).getByTestId(Container.BlockSection)
	).toHaveCount(2);
	await expect(
	...
	await expect(parent.getByTestId(CustomSoftwareDevelopent.ContentSwitcherButton_AiMlDataScience)).not.toHaveClass(
		/active/
	);
	await expect(activeContentSection).toBeHidden();
...
test(`Check the carousel in the 'Custom software development process' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const infoItems = parent.getByTestId(Container.CarouselSection);
	// Check content
	await expect(infoItems).toHaveCount(4);

	// ***data-id fix required

	// testData?.forEach(async function (value, i) {
	// 	const item = infoItems.nth(i)
	// 	await expect(item).toBeVisible();
	// 	await expect(item.getByTestId(Container.SectionTitle).nth(0)).toHaveText(value.title);
	// 	await expect(item.getByTestId(Container.SectionNumber)).toHaveText(value.number);
	// });

	const leftButton = parent.getByTestId(Container.CarouselButtonPrev);
	const rightButton = parent.getByTestId(Container.CarouselButtonNext);
	// Check buttons logic
	await expect(leftButton).toHaveAttribute('data-disabled', 'true');
	await expect(rightButton).toHaveAttribute('data-disabled', 'false');

	for (let i = 0; i < 3; i++) {
		await rightButton.click({timeout: 3000});
	}
	...
	for (let i = 0; i < 3; i++) {
		await leftButton.click({timeout: 3000});
	}
...
test(`Check the member cards in the 'Custom software development experts' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	const testData = [
		{
			id: 0,
			memberPhoto: 'img/servicesExperts/ivan-ieremenko.jpg',
			memberDescription: 'CEO, Software Architect,\nRuns critical initiatives that make products grow',
			memberName: 'Ivan Ieremenko',
			memberLinkLinkedin: 'https://www.linkedin.com/in/ivan-ieremenko/',
			memberLinkBlog: 'https://tech-stack.io/blog/author/ivan_ieremenko/',
			expectedBlogUrl: 'https://tech-stack.com/blog/author/ivan_ieremenko/',
		},
		{
			id: 6,
			memberPhoto: 'img/servicesExperts/dmytro-shtapauk.jpg',
			memberDescription: 'Head of Account Management,\nDrives cross-functional process transformation',
			memberName: 'Dmytro Shtapauk',
			memberLinkLinkedin: 'https://www.linkedin.com/in/shtapauk/',
			memberLinkBlog: 'https://tech-stack.io/blog/author/dmytro_shtapauk/',
			expectedBlogUrl: 'https://tech-stack.com/blog/author/dmytro_shtapauk/',
		},
	];
	const cards = driver
		.getByTestId(CustomSoftwareDevelopent.CustomSoftwareDevelopmentExperts)
		.getByTestId(Container.MemberItem);
	await expect(cards).toHaveCount(7, {timeout: 3000});

	for (const member of testData) {
		await expect(cards.nth(member.id).getByTestId(Container.MemberPhoto)).toHaveAttribute(
			'src',
			member.memberPhoto
		);
		await expect(cards.nth(member.id).getByTestId(Container.MemberRole)).toHaveText(member.memberDescription);
		await expect(cards.nth(member.id).getByTestId(Container.MemberName)).toHaveText(member.memberName);
		await expect(cards.nth(member.id).getByTestId(Button.Linkedin2)).toHaveAttribute(
			'href',
			member.memberLinkLinkedin
		);
		await expect(cards.nth(member.id).getByTestId(Button.Blog)).toHaveAttribute('href', member.memberLinkBlog);

		const [newLIPage] = await Promise.all([
			driver.DriverContext.waitForEvent('page'),
			await cards.nth(member.id).getByTestId(Button.Linkedin2).click(),
		]);
		...
		const [newBlogPage] = await Promise.all([
			driver.DriverContext.waitForEvent('page'),
			await cards.nth(member.id).getByTestId(Button.Blog).click(),
		]);
...
test(`Check the award cards in 'Our approach to software development' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const actualAwardCardImages = driver
		.getByTestId(CustomSoftwareDevelopent.OurApproachToSoftwareDevelopment)
		.getByTestId(Container.AwardCardImage);
...
test(`Check the award cards and reviews in 'Our approach to software development' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	const actualAwardCardImages = parent.getByTestId(Container.AwardCardImage);
	...
	await expect(reviewContainer.nth(0).getByTestId(CustomSoftwareDevelopent.ReviewAuthorPosition)).toHaveText(
		'Bunking'
	);
	await expect(reviewContainer.nth(0).getByTestId(CustomSoftwareDevelopent.ReviewText)).toHaveText(
		"Techstack Ltd's efforts have helped streamline the development process, allowing them to launch the project earlier than projected."
	);

	await expect(reviewContainer.nth(1).getByTestId(CustomSoftwareDevelopent.ReviewAuthorName)).toHaveText('Executive');
	await expect(reviewContainer.nth(1).getByTestId(CustomSoftwareDevelopent.ReviewAuthorPosition)).toHaveText(
		'Peer-to-Peer Rental Marketplace'
	);
	await expect(reviewContainer.nth(1).getByTestId(CustomSoftwareDevelopent.ReviewText)).toHaveText(
		"Their willingness to help and involvement in the project were also key features to the project's success."
	);
	...
	expect(newLIPage.url()).toContain('https://clutch.co/profile/techstack');
	newLIPage.close();
});
...
test(`Check the 'Related artiles' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	const expectedAwards = [
		{
			id: 0,
			articleLeftInfo: 'Internet of Things',
			articleRightInfo: 'May 24',
			articleTitle: 'The Benefits & Challenges of Integrating Digital Twins with IoT',
		},
		{
			id: 1,
			articleLeftInfo: 'Software Development',
			articleRightInfo: 'Mar 20',
			articleTitle: 'Using GraalVM in a Real-world Scenario: Techstack’s Experience',
		},
		{
			id: 2,
			articleLeftInfo: 'Processes Architecture',
			articleRightInfo: 'Feb 22',
			articleTitle: 'The Best Practices and Impact of Code Review on Productivity and Timelines',
		},
	];
	const actualArticles = driver
		.getByTestId(CustomSoftwareDevelopent.RelatedArticles)
		.getByTestId(CustomSoftwareDevelopent.ArticleItem);
	for (const award of expectedAwards) {
		await expect(actualArticles.nth(award.id).getByTestId(CustomSoftwareDevelopent.ArticleLeftInfo)).toContainText(
			award.articleLeftInfo
		);
		await expect(actualArticles.nth(award.id).getByTestId(CustomSoftwareDevelopent.ArticleRightInfo)).toContainText(
			award.articleRightInfo
		);
		await expect(actualArticles.nth(award.id).getByTestId(CustomSoftwareDevelopent.ArticleTitle)).toHaveText(
			award.articleTitle
		);
	}
});
...
test(`Check the 'FAQ' section of the 'Custom software development' page @Regression @CustomSoftwareDevelopent @TSWEB-672`, async () => {
	...
	await expect(sections.nth(0).getByTestId(Container.SectionTitle)).toHaveText(
		'Why choose Techstack for custom software development?'
	);
	await expect(sections.nth(0).getByTestId(CustomSoftwareDevelopent.SectionShortAnswer)).toHaveText(
		'Our developers, QA engineers, and managers work closely with your team to ensure that the software solution meets your specific business needs'
	);
	await expect(sections.nth(0).getByTestId(CustomSoftwareDevelopent.SectionShortAnswer)).toBeVisible();
	await expect(sections.nth(0).getByTestId(CustomSoftwareDevelopent.SectionFullAnswer)).toHaveText(
		'. They are committed to delivering quality code that is well-tested and scalable. In addition, our developers are always up-to-date on the latest technology trends, so you can be confident that your solution will be built using the best possible tools and techniques. As a result, you can focus on running your business, while we handle the development process.'
	);
	...
	await expect(sections.nth(4).getByTestId(CustomSoftwareDevelopent.SectionShortAnswer)).toHaveText(
		'Software development is a process of creating, testing, and maintaining software. The custom software development process is a bit different from the traditional one as it is tailored according'
	);
	await expect(sections.nth(4).getByTestId(CustomSoftwareDevelopent.SectionFullAnswer)).toHaveText(
		" to the client's specific needs. In this type of development, a team of experts works closely with the client to understand their requirements and develop a solution that fits their needs. Then, we set up a team and processes to deliver the custom solution. We seamlessly monitor the product and team’s health weekly within a unique framework and analyze problem zones to drive a constant improvement process."
	);
