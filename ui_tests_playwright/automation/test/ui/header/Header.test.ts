...
for (const url of testDataProvider) {
    ...
    test(`Hovering the mouse over the menu buttons highlights the buttons text in #FFC600 color in the 'Header' on the '${url}' link @Regression @Header @TSWEB-656`, async () => {
        ...
        const menuHeaderslist = [driver.getByTestId(Button.Menu_Services),
    ...
    test(`Check the redirection for the Services block in the 'Header' on the '${url}' link @Regression @Header @TSWEB-656`, async () => {
        await baseDriverSteps.goToUrl(url);
        const servicesList = [Button.Services_OurServices, Button.Services_CustomDev, Button.Services_CloudAndDev, Button.Services_BigData,
        Button.Services_AiMl, Button.Services_InternetOfThings, Button.Services_MobileDev, Button.Services_UiUxDesign, Button.Services_QaAsAServ, Button.Services_ConsultingServ];
    ...
    test(`Check the redirection for the Company block in the 'Header' on the '${url}' link @Regression @Header @TSWEB-656`, async () => {
        ...
        const companyList = [Button.Company_AboutUs, Button.Company_HowWeWork, Button.Company_Career, Button.Company_CaseStudies,
        Button.Company_Blog];
