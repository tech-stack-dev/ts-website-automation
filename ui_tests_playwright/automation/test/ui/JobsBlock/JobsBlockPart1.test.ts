...
test('Check that breadcrumbs displays correctly on job page @Regression @JobsBlock @TSWEB-560 @TSWEB-560', async () => {
...
test('Check localization on job page @Regression @JobsBlock @TSWEB-560 @TSWEB-560', async () => {
	await careerSteps.verifyThatCareerWasCreated(`JobsBlockTest${sessionValue.stringValue.toLocaleUpperCase()}`);
	await careerSteps.clickOnCareerCard(`JobsBlockTest${sessionValue.stringValue.toLocaleUpperCase()}`);
...
test('Check that user can switch language in navigation header in job page @Regression @JobsBlock @TSWEB-146 @TSWEB-574 @TSWEB-560', async () => {
	await careerSteps.verifyThatCareerWasCreated(`JobsBlockTest${sessionValue.stringValue.toLocaleUpperCase()}`);
	await careerSteps.clickOnCareerCard(`JobsBlockTest${sessionValue.stringValue.toLocaleUpperCase()}`);
