...
test('Check the container title and number from the "Get a Quote" page @desktop @mobile @Regression @GetAQuote @TSWEB-1766',
	async () => {
...
test('Check partner logos in "Trusted By" container from the "Get a Quote" page @desktop @mobile @Regression @GetAQuote @TSWEB-1766',
	async () => {
		const trustedByContainer = driver.getByTestId(GetAQuote.TrustedBy);
		await trustedByContainer.scrollIntoViewIfNeeded();
		const partnerLogos = [MainSiteImages.Corel, MainSiteImages.SkillLabLogo, MainSiteImages.VivaQuantLogo, MainSiteImages.ExaumLogo];
